...
//	this.angle = angle;
	this.fromRad = 0;
	me.attr = { stroke: '#000' }; //colour lines format orig: , 'stroke-dasharray': "-"
	this.toRad; //colour lines orig: 888


	this.getXYline = function() {
		me.toRad = me.rad;
		this.x1 = this.o.x + Math.cos(this.angle) * this.fromRad; 
		this.y1 = this.o.y -Math.sin(this.angle) * this.fromRad;
		this.x2 = this.o.x + Math.cos(this.angle) * this.toRad; 
	...
	me.init = function() {
		...
		me.getXY();
		this.getXYline();
		//console.log("foo", "M"+this.x1+" "+this.y1+"L"+this.x2+" "+this.y2, this.attr);
		this.path = this.bc.paper.path(
			"M"+this.x1+" "+this.y1+"L"+this.x2+" "+this.y2
		).attr(this.attr);
	...
	me.show = function() {
		...
		me.dashedBorder = me.paper.circle(cx, cy, r-3)
			.attr({ stroke: '#fff', 'stroke-dasharray': "- " }); //colour: circle border orig:, 'stroke-dasharray': "- "
		//colour: circle border orig: ffffff 
		
		me.label = $('<div class="bubbletree-label '+me.node.id+'"><div class="bubbletree-amount">'+me.node.shortLabel+'</div></div>');
		...
		//art mode comment out me.label2
